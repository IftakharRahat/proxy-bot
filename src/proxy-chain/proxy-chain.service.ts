...
export class ProxyChainService implements OnModuleInit {
     ...
     */
    async rebuildConfig() {
        this.logger.log('Rebuilding 3proxy configuration with External Auth File...');
        ...
        // Write to a separate auth file to prevent config bloat
        // user:CL:pass user2:CL:pass2 ...
        const authContent = usersList.join(' ');
        const authFilePath = '/etc/3proxy/.proxyauth';

        if (process.platform !== 'win32') {
            try {
                await fs.promises.writeFile(authFilePath, authContent);
                // Secure the auth file
                await execAsync(`chmod 600 ${authFilePath}`);
            } catch (err) {
                this.logger.error(`Failed to write auth file: ${err.message}`);
            }
        }
...
# --- Global Auth Check ---
# We use an external file included in each block
# Path: ${authFilePath}
...
`;
        ...
        for (const port of sharedPorts) {
            if (port.upstreamHost && port.upstreamPort && port.localPort) {
                ...
                config += `flush\n`;
                // Include users from external file
                config += `users $${authFilePath}\n`;
                config += `auth strong\n`;
        ...
        this.logger.log('Generated 3proxy config using External Auth File pattern.');
        ...
        try {
            if (process.platform !== 'win32') {
                ...
                this.logger.log('3proxy nuclear-restarted. status:\n' + stdout);
            } else {
